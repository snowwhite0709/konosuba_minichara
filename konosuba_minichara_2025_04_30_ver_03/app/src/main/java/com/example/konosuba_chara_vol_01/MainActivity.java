...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveButton.setOnClickListener(v -> {
            ...
            // まだ未登録のBIG番号で、かつ正しい順番なら新規登録
            else {
                if (nextBigNumber != totalBigCount) {
                    // 例：BIG1→BIG3と飛んでいた場合など、連続でないならNG
        ...
        resetButton.setOnClickListener(v -> {
            // vol番号を＋1して、新しいファイル名に切り替える
            // → 別の台として新しい記録ファイルを使う想定
            incrementVol();
            // 現在のキャラクター出現カウント配列を取得
            int[] counts = adapter.getCounts();
            // 全キャラのカウントを0にリセット
            for (int i = 0; i < counts.length; i++) {
                counts[i] = 0;
            }
            // BIG回数、メインキャラ率などを含む集計表示を更新
            updateAllSummaries(adapter, bigCountTextView);

            // SharedPreferencesも初期化して保存
            adapter.saveCountsToPrefs(MainActivity.this);

            // RecyclerView（各キャラの＋／－ UI）もリセット状態で再表示
            adapter.notifyDataSetChanged();

            // ユーザーにリセット完了を通知（トースト表示）
            Toast.makeText(MainActivity.this, "データをリセットしました", Toast.LENGTH_SHORT).show();
        });


        // ボタン・表示エリア取得
        Button allDisplayButton = findViewById(R.id.allDisplayButton);
        Button summaryDisplayButton = findViewById(R.id.summaryDisplayButton);
        ScrollView outputScrollView = findViewById(R.id.outputScrollView);
        ...
        allDisplayButton.setOnClickListener(v -> {
            if (isShowingAll) {
                ...
                isShowingAll = false;
                allDisplayButton.setText("各BIG累計確率全件表示");
            } else {
                // ファイル全体を表示する
                String content = readFileContent(currentFileName);
                outputTextView.setText(content);
                outputScrollView.setVisibility(View.VISIBLE);
                isShowingAll = true;
                isShowingSummary = false; // もう一方のトグルもオフに
                summaryDisplayButton.setText("各BIG累計確率表示");
                allDisplayButton.setText("非表示");
            }
        });
        ...
        summaryDisplayButton.setOnClickListener(v -> {
            if (isShowingSummary) {
                ...
                // 合計のみ抽出表示
                String summary = readSummaryOnly(currentFileName);
                outputTextView.setText(summary);
                outputScrollView.setVisibility(View.VISIBLE);
                isShowingSummary = true;
                isShowingAll = false;
                allDisplayButton.setText("各BIG累計確率全件表示");
                summaryDisplayButton.setText("非表示");
            }
        });
    ...
    private String readFileContent(String fileName) {
        ...
        if (!file.exists()) return "ファイルが存在しません。";

        System.out.println("*****************************************************");
        System.out.println("fileName in readFileContent: " + fileName);
        System.out.println("*****************************************************");
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            builder.append("読み込みエラー: ").append(e.getMessage());
        }

        System.out.println("*****************************************************");
        System.out.println("builder.toString() in readFileContent: " + builder.toString());
        System.out.println("*****************************************************");
    ...
    // 「メインキャラ〜確定系」だけを抽出
    private String readSummaryOnly(String fileName) {
        File file = new File(getFilesDir(), fileName);
        if (!file.exists()) return "ファイルが存在しません。";

        System.out.println("*****************************************************");
        System.out.println("fileName in readSummaryOnly: " + fileName);
        System.out.println("*****************************************************");


        StringBuilder builder = new StringBuilder();
        boolean inSummaryBlock = false;
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith("【BIG")) {
                    // BIGの見出しを出力し、summaryブロック開始を待つ
                    builder.append("\n").append(line).append("\n\n");
                    inSummaryBlock = false;
                } else if (line.contains("メインキャラ")) {
                    builder.append(line).append("\n");
                    inSummaryBlock = true;
                } else if (inSummaryBlock &&
                        (line.contains("サブキャラ") || line.contains("サキュバス") || line.contains("確定系"))) {
                    builder.append(line).append("\n");
